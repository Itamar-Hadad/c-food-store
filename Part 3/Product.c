...
int writeNameBarcodeProduct_Comp(const Product* pProduct,FILE* fp) {
	int nameSize = (int)strlen(pProduct->name);
	BYTE temp[4];
	//FR23456
	//barcode numbers
	temp[0] = (pProduct->barcode[2]- '0') << 4;
	temp[0] |= pProduct->barcode[3] - '0';
	temp[1] = (pProduct->barcode[4] - '0') << 4;
	temp[1] |= pProduct->barcode[5] - '0';
	temp[2] = (pProduct->barcode[6] - '0') << 4;

	//type
	temp[2] |= pProduct->type << 2;
	//name
	temp[2] |= nameSize >> 2;
	temp[3] = nameSize << 6;

	if(fwrite(temp, sizeof(BYTE), 4, fp) != 4) {
		printf("Error write product: barcode, type & name size\n");
		return 0;
	}
	if(fwrite(pProduct->name, sizeof(char), nameSize, fp) != nameSize) {
		printf("Error write product name\n");
		return 0;
	}
	return 1;
}
...
int writeAmountPriceProduct_Comp(const Product* pProduct, FILE* fp) {
	BYTE temp[3];
	temp[0] = pProduct->count;
	int floatPart = (int)(pProduct->price * 100) % 100;
	int intPart = (int)pProduct->price;
	temp[1] = floatPart << 1;
	temp[1] |= intPart >> 8;
	temp[2] = intPart;

	if(fwrite(temp, sizeof(BYTE), 3, fp) != 3) {
		printf("Error write product: amount & price\n");
		return 0;
	}
	return 1;
}

int readNameBarcodeProduct_Comp(Product* pProduct, FILE* fp) {
	BYTE temp[4];
	if(fread(temp, sizeof(BYTE), 4, fp) != 4) {
		printf("Error read product: barcode, type & name size\n");
		return 0;
	}
	int nameSize = (temp[2] & 0x3) << 2 | ((temp[3] >> 6) & 0x3);
	pProduct->type = (temp[2] >> 2) & 0x3;
	strcpy(pProduct->barcode, typePrefix[pProduct->type]);
	pProduct->barcode[2] = (temp[0] >> 4 & 0xF) + '0';
	pProduct->barcode[3] = (temp[0] & 0xF) + '0';
	pProduct->barcode[4] = (temp[1] >> 4 & 0xF) + '0';
	pProduct->barcode[5] = (temp[1] & 0xF) + '0';
	pProduct->barcode[6] = (temp[2] >> 4 & 0xF) + '0';
	pProduct->barcode[7] = '\0';
	if(fread(pProduct->name, sizeof(char), nameSize, fp) != nameSize) {
		printf("Error read product name\n");
		return 0;
	}
	pProduct->name[nameSize + 1] = '\0';
	return 1;
}

int readAmountPriceProduct_Comp(Product* pProduct, FILE* fp) {
	BYTE temp[3];
	if(fread(temp, sizeof(BYTE), 3, fp) != 3) {
		printf("Error read product: amount & price\n");
		return 0;
	}
	pProduct->count = temp[0] & 0xFF;
	float floatPart = (float)((temp[1] >> 1) & 0x7F);
	float integerPart = (float)((temp[1] & 0x1) | (temp[2] & 0xFF));
	pProduct -> price = integerPart + floatPart / 100;
	return 1;
}
